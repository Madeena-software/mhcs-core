...
| MVP-GAP-009 | Operator Portal not yet complete | Operator Portal | The bounded foundation, arrival, front-desk identity-verification, and paper-consent slice is implemented; the complete Operator MVP remains unavailable | Expose only the bounded authenticated site, attendance, arrival, verification, and paper-consent slice | MVP-04 | open | Complete bounded Operator MVP and authorization evidence pass | This task does not close the gap; check-in, ticketing, queue, general consent, and clinical behavior remain out of scope. |
...
| MVP-GAP-012 | Queue and attendance workflow incomplete | Operator Portal | The bounded attendance, arrival, identity-verification, and paper-consent foundation exists; check-in, ticketing, queue, and later examination stages remain unavailable | Do not expose queue, ticket, check-in, or clinical behavior | MVP-04 | open | Remaining queue, check-in, ticket, attendance, and audit behavior passes | MVP-04C adds paper consent only; it does not advance a booking beyond `arrived`. |
