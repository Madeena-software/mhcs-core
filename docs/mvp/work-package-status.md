...
private and is the only internal network service boundary. Requirement
assignments and module ownership remain unchanged.
...
| WP-24 | Image Gateway AI, publication, and replacement studies | IMG-034..IMG-046, IMG-058..IMG-059 | No AI, publication, report-return, or replacement-study workflow is present | MVP-06/MVP-07 on `mhcs-core` `main`; Image Gateway owns AI, publication, and replacement-study provider behavior | not-started | Entire package | Requirement assignments and Image Gateway ownership are unchanged. Approved contracts may precede consumers, but both provider and consumers are implemented sequentially on `main` under their module ownership; no automated report return or clinical AI claim follows from a contract alone. |
