...
Workstream labels describe parallel delivery ownership only; they do not change
Work Package requirement assignments or module ownership. The Image Gateway
branch must merge into `main`, and the merged integration gate must pass, before
final beta completion.
...
| WP-07 | Member clinical, consent, attendance, walk-in, and cash contracts | MEM-068..MEM-083, MEM-086..MEM-096, MEM-102..MEM-107, MEM-215, MEM-221..MEM-222 | The bounded Member attendance/arrival, paper-consent confirmation, and `arrived` to `checked_in` contracts consumed by MVP-04 are present; no clinical assessment, general consent administration, walk-in, or cash workflow is present | MVP-04 and later | not-started | Entire package except the exact bounded attendance/arrival, paper-consent, and check-in contracts consumed by MVP-04 | This remains not-started as a Work Package; the narrow contracts do not establish clinical workflow or general consent capability. |
...
| WP-14 | Operator protocol, NPZ drafts, and complete capture submission | OPR-031..OPR-046, OPR-132, OPR-137 | No Operator capture workflow; only generic Image Gateway input policy exists | Main workstream; contract-independent protocol and draft work may proceed, while complete submission consumes the approved merged `AcceptCompleteCaptureSet` contract | partially-implemented | Device schema, drafts, capture submission, and business contract | Operator ownership and requirement assignments are unchanged. The Gateway workstream may merge the approved contract and fixtures before completing WP-23; Gateway internals remain Image Gateway workstream work. |
| WP-15 | Operator AI status, corrections, repeats, and read-only images | OPR-047..OPR-060 | No Operator status monitor, viewer, correction, or repeat handoff is present | Main workstream; consumer work may proceed against approved merged Gateway status and publication contracts | not-started | Entire package | Operator ownership and requirement assignments are unchanged. Runtime integration and end-to-end completion remain deferred until the Gateway implementation merges. |
...
| WP-17 | Operator administration, contracts, and FHIR boundary | OPR-074..OPR-099, OPR-108..OPR-116, OPR-134, OPR-136, OPR-138 | Bounded Operator site, profile, site-assignment, eligible-shift, shift-assignment, arrival, and private paper-ticket resources plus local application contracts are implemented; FHIR and broader operational administration are absent | Main workstream for Operator ownership; MVP-06 administration may depend on merged Gateway operational contracts | partially-implemented | Remaining contracts, FHIR, protocol, queue, ticket policy, and operational administration | Requirement assignments and Operator ownership are unchanged. The shared administrator interface may present Operator capabilities without owning Operator records or rules; pre-merge Gateway dependencies are deferred. |
...
| WP-23 | Image Gateway storage, manifests, MPIPS, and processing | ARCH-019..ARCH-027, ARCH-042, ARCH-044, IMG-001..IMG-033, IMG-047..IMG-057 | Image Gateway security policy, manifest gate, and worker boundary exist; no durable storage, MPIPS adapter, processing workflow, or operational administration surface exists | Image Gateway workstream: contract-first `AcceptCompleteCaptureSet` slice, then MVP-05/MVP-06 Gateway flows and operational administration | partially-implemented | Durable object storage, actual processing, MPIPS contract, retries, retention, operational administration, and production policy | Requirement assignments and Image Gateway ownership are unchanged. An approved contract and fixtures may merge before the complete provider; final integration waits for the provider implementation. |
