...
The examination stores blood pressure, weight, one height value in centimetres, temperature, GCU results and fasting context, three optional PEF attempts, the maximum valid PEF attempt, and notes. BMI is calculated from the persisted weight and height. Microtoise is shown only as height-measurement method context; it is not a second numeric result. No clinical thresholds or interpretations are added.

The protected PDF route loads the saved MCU record and canonical participant identity, includes the screening disclaimer and manual signature areas, and supports browser print/view and attachment download. The existing mPDF dependency is reused.
