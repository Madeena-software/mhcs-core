...
- Do not modify application code, routes, tests, migrations, dependencies, or runtime configuration.
- Preserve all existing Work Packages, their numbering, requirement assignments, task files, and historical evidence.
- Do not commit, stage, deploy, access production, or perform any production-affecting operation.
...
2. Validate the baseline and inspect repository state.
3. Read all required context and Work Package evidence.
4. Check for overlap or conflicts in `docs/mvp/**`.
5. Create the six required documentation files.
6. Run the required documentation checks.
7. Re-read the task and report the outcome.
...
- Method: Run `git diff --check` and verify the six required files and allowed-path boundary with Git.
- Expected result: The check passes, all six files exist, and no file outside `docs/mvp/**` is modified.
