...
<section aria-labelledby="xray-capture-title">
    ...
    <p class="muted">{{ __('Radiography admission') }}: <code>{{ $form['ticket_number'] }}</code>; @if (!empty($form['locator_code'])) {{ __('Session locator code') }}: <code>{{ $form['locator_code'] }}</code>; @endif {{ __('Upload one radiograph NPZ and its matching gain NPZ.') }}</p>
