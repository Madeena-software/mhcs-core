...
final class ShiftScheduleResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            Section::make('Jadwal Sesi Foto Radiografi')->schema([
                Select::make('examination_site_id')->label('Lokasi')->options(fn (): array => \App\Modules\Member\Domain\Models\ExaminationSiteReference::query()->where('active', true)->orderBy('display_name')->pluck('display_name', 'id')->all())->required()->searchable(),
                Select::make('service_offering_id')->label('Layanan')->options(fn (): array => \App\Modules\Member\Domain\Models\ServiceOffering::query()->where('active', true)->orderBy('name')->pluck('name', 'id')->all())->required()->searchable(),
                TextInput::make('starts_at')->label('Mulai (ISO 8601 dengan offset)')->required()->placeholder('2030-01-10T10:00:00+07:00'),
                TextInput::make('ends_at')->label('Selesai (ISO 8601 dengan offset)')->required()->placeholder('2030-01-10T11:00:00+07:00'),
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('confirmed_count')->label('Terkonfirmasi')->state(fn (ShiftSchedule $record): int => $record->bookings()->whereIn('status', \App\Modules\Member\Application\Services\Mvp03BookingService::capacityStatuses())->count()),
        ])->actions([])->headerActions([])->toolbarActions([])->defaultSort('starts_at')->emptyStateHeading('Belum ada jadwal');
    }
    ...
    public static function canViewAny(): bool { return self::authorized('scheduleRead'); }
    public static function canView(Model $record): bool { return self::canViewAny(); }
    public static function canCreate(): bool { return self::authorized('scheduleManage'); }
    public static function canEdit(Model $record): bool { return self::authorized('scheduleManage'); }
